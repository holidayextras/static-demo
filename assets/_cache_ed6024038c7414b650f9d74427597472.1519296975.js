...
var lv_target = $(this).data('target');
if( lv_target && $(lv_target).length < 1 ) {
var width = $(this).data('width') ? $(this).data('width') : '400';
var height = $(this).data('height') ? $(this).data('height') : '450';
var lv_url = $(this).attr('href');
var lv_title = $(this).attr('title') ? $(this).attr('title') : lv_target.replace(/#/, '').replace(/(-)|(_)/g, ' ');
$('body').append('<div class="modal fade" style="width:'+(parseInt(width))+'px; margin:-'+(parseInt(height)/2)+'px 0 0 -'+(parseInt(width)/2)+'px;" id="'+lv_target.replace(/#/,'')+'"><div class="modal-header"><button class="close" data-dismiss="modal">x</button><h3>'+lv_title+'</h3></div><div class="modal-body"></div><div class="modal-footer"><a data-dismiss="modal" class="close btn">Close</a></div></div>');
$.ajax({
url: lv_url,
success: function(data){
data = data.split('</body>')[0];
data = data.split('<body>')[1];
$('.modal-body', $(lv_target)).append(data);
}
});
}
});
var container = $('body');
container.append('<div id="x"></div>');
$('#x').html('x').css({'position':'fixed','top':'0'});
window.scrollTo(1000,1000);
var isSupported = $('#x').position().top;
$('#x').remove();
window.scrollTo(0,1);
if( !isSupported && window.orientation && ((window.screen.height < 480 && window.orientation == 0) || ( window.screen.height <= 260 && window.orientation == 90)) ) {
...
window.addEventListener("load", updateEmailCapture, false);
else if (window.attachEvent)
window.attachEvent("onload", updateEmailCapture);
else
window.onload = updateEmailCapture;
function updateEmailCapture() {
var setEmailCaptureCookie = function(email) {
if( typeof hxtrack_inline != 'undefined' ) {
hxtrack_inline.track('flight_tripPremium', email);
}
if( typeof $.cookie != 'undefined' ) {
$.cookie('Email', email, { path: '/' , expires: (email?90:-1), domain: window.location.host.replace(/.+?\./, '')});
}
};
...
if( typeof $.cookie != 'undefined' && ($.cookie('Email') || getURLParams('email') || getURLParams('email_address')) ) {
var urlEmail = getURLParams('email');
if( !urlEmail ) urlEmail = getURLParams('email_address');
var email = (urlEmail?urlEmail:$.cookie('Email'));
setEmailCaptureCookie(email);
}
if( window.location.host.indexOf('.holidayextras.co.uk') > -1 ) {
...
setEmailCaptureCookie(data.email);
hxtrack_inline.track('loggedinuser', data.email);
$(':input[type=email].emailAddress').val(data.email);
}
});
